...
export function createWebAdapter(): Commands {
  return {
    ...
    renameBranch: (repoPath, oldName, newName) =>
      post("/branch/rename", { repoPath, oldName, newName }),
    ...
    resetToCommit: (repoPath, commitId, mode) =>
      post("/commit/reset", { repoPath, commitId, mode }),
    ...
    getUnpushedCommits: (repoPath, remote?, branch?) =>
      post("/unpushed-commits", { repoPath, remote, branch }),
    ...
    fetchBranch: (repoPath, remote, branchName) =>
      post("/branch/fetch-update", { repoPath, remote, branchName }),
    ...
    stashSave: (repoPath, message, includeUntracked) =>
      post("/stash/save", { repoPath, message, includeUntracked }),
    ...
    getStashFileDiff: (repoPath, index, filePath) =>
      post("/stash/file-diff", { repoPath, index, filePath }),
    ...
    resolveConflict: (repoPath, filePath, content) =>
      post("/conflict/resolve", { repoPath, filePath, content }),
    getWorkingFileContent: (repoPath, filePath) =>
      post("/conflict/working-content", { repoPath, filePath }),
    getMergeState: (repoPath) => post("/merge-state", { repoPath }),
    continueOperation: (repoPath, op) => post("/merge-op/continue", { repoPath, op }),
    abortOperation: (repoPath, op) => post("/merge-op/abort", { repoPath, op }),
    cloneRepo: (url, path) => post("/clone", { url, path }),
    getFileContent: (repoPath, commitId, filePath) =>
      post("/file/content", { repoPath, commitId, filePath }),
    discardFileChanges: (repoPath, filePath) => post("/file/discard", { repoPath, filePath }),
    getFileDiffRaw: (repoPath, filePath, staged) =>
      post("/file/diff-raw", { repoPath, filePath, staged }),
    deleteFile: (repoPath, filePath) => post("/file/delete", { repoPath, filePath }),
    stashFile: (repoPath, filePath, message?) =>
      post("/stash/file", { repoPath, filePath, message }),
  };
}
