...
export function createElectronAdapter(): Commands {
  ...
  return {
    ...
    getCommitDiff: (repoPath, commitId, filePath) =>
      api.invoke("get_commit_diff", repoPath, commitId, filePath),
    getFileDiff: (repoPath, filePath, staged) =>
      api.invoke("get_file_diff", repoPath, filePath, staged),
    compareCommits: (repoPath, fromId, toId) =>
      api.invoke("compare_commits", repoPath, fromId, toId),
    getBranches: (repoPath) => api.invoke("get_branches", repoPath),
    createBranch: (repoPath, name, startPoint?) =>
      api.invoke("create_branch", repoPath, name, startPoint),
    ...
    renameBranch: (repoPath, oldName, newName) =>
      api.invoke("rename_branch", repoPath, oldName, newName),
    ...
    resetToCommit: (repoPath, commitId, mode) =>
      api.invoke("reset_to_commit", repoPath, commitId, mode),
    ...
    getUnpushedCommits: (repoPath, remote?, branch?) =>
      api.invoke("get_unpushed_commits", repoPath, remote, branch),
    ...
    fetchBranch: (repoPath, remote, branchName) =>
      api.invoke("fetch_branch", repoPath, remote, branchName),
    getRemotes: (repoPath) => api.invoke("get_remotes", repoPath),
    getStashList: (repoPath) => api.invoke("get_stash_list", repoPath),
    stashSave: (repoPath, message, includeUntracked) =>
      api.invoke("stash_save", repoPath, message, includeUntracked),
    ...
    getStashFileDiff: (repoPath, index, filePath) =>
      api.invoke("get_stash_file_diff", repoPath, index, filePath),
    getBlame: (repoPath, filePath, commitId?) =>
      api.invoke("get_blame", repoPath, filePath, commitId),
    getConflictFiles: (repoPath) => api.invoke("get_conflict_files", repoPath),
    getConflictContent: (repoPath, filePath) =>
      api.invoke("get_conflict_content", repoPath, filePath),
    resolveConflict: (repoPath, filePath, content) =>
      api.invoke("resolve_conflict", repoPath, filePath, content),
    getWorkingFileContent: (repoPath, filePath) =>
      api.invoke("get_working_file_content", repoPath, filePath),
    ...
    getFileContent: (repoPath, commitId, filePath) =>
      api.invoke("get_file_content", repoPath, commitId, filePath),
    discardFileChanges: (repoPath, filePath) =>
      api.invoke("discard_file_changes", repoPath, filePath),
    getFileDiffRaw: (repoPath, filePath, staged) =>
      api.invoke("get_file_diff_raw", repoPath, filePath, staged),
    deleteFile: (repoPath, filePath) => api.invoke("delete_file", repoPath, filePath),
    stashFile: (repoPath, filePath, message?) =>
      api.invoke("stash_file", repoPath, filePath, message),
  };
}
