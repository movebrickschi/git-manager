...
describe("translateGitError", () => {
  ...
  describe("通过原始 stderr 关键字翻译", () => {
    it("non-fast-forward", () => {
      expect(translateGitError("Updates were rejected because the tip is behind: non-fast-forward")).toMatch(/pull/);
    });
    it("could not resolve host", () => {
      expect(translateGitError("fatal: unable to access 'x': Could not resolve host: github.com")).toMatch(/无法解析|网络/);
    });
    it("index.lock", () => {
      expect(translateGitError("fatal: Unable to create '/.git/index.lock': File exists.")).toMatch(/锁定/);
    });
    it("destination already exists", () => {
      expect(translateGitError("fatal: destination path 'foo' already exists and is not an empty directory.")).toMatch(/已存在/);
    });
  });
  describe("边界", () => {
    it("空字符串返回 \"未知错误\"", () => {
    ...
    it("未匹配模式 → 原文兜底", () => {
      expect(translateGitError("some random unknown text 中文混杂")).toBe("some random unknown text 中文混杂");
    });
    it("string 直接传入", () => {
